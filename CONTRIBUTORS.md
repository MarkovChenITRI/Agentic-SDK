...
一列代表一段隸屬關係，不是一個人。同一個帳號可能出現在多列：換部門時新增一列
並在舊列補上結束月份，代表兩個單位貢獻時則兩列的期間重疊。既有的列不改寫——
那段期間的貢獻歸屬於當時的單位，事後不因人事異動而改變。
...
| [@MarkovChenITRI](https://github.com/MarkovChenITRI) | Markov Chen | 機械所 機器人技術組（Q 組） | 2026-09 起 | 即時語音互動 |
| [@Terrykuo20031222](https://github.com/Terrykuo20031222) | 未定 | 未定 | 2026-08 | 合併分支 |

期間只寫起訖月份。人離開該單位時填上結束月份，另起一列記錄新單位；
同時代表兩個單位貢獻時，兩列各自記錄，期間可以重疊。
...
同一份清單以機器可讀的形式記在 `REUSE.toml`，CI 會檢查其中的路徑是否仍指到
實際檔案。技轉作業界定標的時以那一份為準，下表供閱讀。
