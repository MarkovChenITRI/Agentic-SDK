...
| 層 | 是什麼 | 型別 |
| --- | --- | --- |
| Workflow | 執行的容器。決定下一步跑哪個模組、管住執行上限、送出事件、回傳結果 | `Workflow`、`WorkflowResult` |
| Module | 一步工作的單位。凡是有名稱、且吃狀態回輸出的物件都算 | `Module` 協定 |
| Memory | 跨輪的對話記錄。同一個 `session_id` 的前後文由它承接，所有模組讀得到 | `MemoryStore`、`InContextMemory`、`PersistentMemory` |
| Data | 模組之間傳遞的東西。一輪內的共用狀態，加上每個模組交出的結果 | `WorkflowState`、`ModuleOutput` |

四層的分工決定了擴充的成本。寫一個新模組只需要滿足 `Module` 協定——吃一個 `WorkflowState`、
回一個 `ModuleOutput`——不必碰 Workflow 的迴圈，也不必自己管記憶怎麼存。換一種記憶實作只需要
滿足 `MemoryStore` 介面，模組那一層完全不動。

Workflow 也不是把模組排好順序跑一遍就結束。每個模組回傳的 `ModuleOutput` 裡帶著下一步是誰，
所以同一輪裡可以先查一次資料、發現不夠再查一次，也可以在檢查沒過之後重新規劃。
停下來的時機由模組自己決定，另有步數上限擋住繞不出來的情況。

## 只換其中一步

Module 這一層預設裝著五個步驟，對應論文架構裡的四個機制加上獨立出來的執行。
每一步都附了現成的實作，換掉想自己動手的那一個，其餘四步沿用現成的。
...
差別只在每一步換上哪個模組。要做語音助理就把 Perceive 換成語音、Action 換成語音回答，
中間三步完全不動。

想改的通常只有一步，但那一步要跑起來，前後四步一個都不能少。以下三種是實務上常見的情形。
