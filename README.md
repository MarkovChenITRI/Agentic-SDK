...
Agentic SDK 是一個 Python 程式庫，用來組裝 AI agent 從收到問題到給出回答的完整流程。這條流程固定拆成五個步驟，每一步是一個獨立的 Python 物件，步驟之間怎麼傳資料由程式庫承接，不必自己接線。想改其中一步的人因此不必先把其餘四步造出來：換掉那一個物件，另外四步照舊運作。
...
把流程拆成五步，是為了讓一個想法可以單獨被驗證。新的點子通常只落在其中一步，可能是一顆新的推論晶片、一種新的規劃演算法，或一套自己的業務邏輯；但要證明它有效，得先有一條完整的流程把它跑起來。這個程式庫提供其餘四步，換上自己的那一步就能開始量。
