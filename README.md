...
Agentic SDK 是一個 Python 程式庫，把一個 AI agent 的執行拆成四層，每一層都附現成的實作，
也都可以換成自訂的實作。架構取自 Park 等人 2023 年的
[Generative Agents](https://arxiv.org/abs/2304.03442)。

| 層 | 負責什麼 |
| --- | --- |
| Workflow | 跑完一輪。決定下一步走到哪個模組、管住執行上限、把每一步的進度送出去 |
| Module | 做一步工作。一個模組收到當下的狀態，做完事情交出結果與下一步是誰 |
| Memory | 記住前面談過什麼。同一個對話的前後文由它承接，每個模組都讀得到 |
| Data | 在模組之間傳遞。一輪之內共用同一份狀態，模組各自往上面添東西 |

四層各自替換，互不牽動。加一個自訂步驟不會動到執行的迴圈與記憶的存法，
換一種記憶的存法不會動到任何模組。

跑完一輪也不是把模組排好順序走一遍。每個模組做完會交代下一步是誰，
所以同一輪裡可以先查一次資料、發現不夠再查一次，也可以在檢查沒過之後重新規劃。
停下來的時機由模組決定，另有步數上限擋住繞不出來的情況。

## 只換其中一步

Module 這一層預設裝著五個步驟，對應該架構的四個機制加上獨立出來的執行。
每一步都附現成的實作，換掉其中一步時，其餘四步沿用現成的。
...
專案附一個網頁版的 Playground，不寫程式也能使用。在頁面上選定每一步用哪個模組、
填入知識庫內容，即可與組出來的 agent 對話；結果可以存成一個 agent 重複使用，
也可以匯出成 Python 程式碼。
...
三個例子對應上面那三種替換。第一個不使用任何模型服務，複製貼上即可執行；第二個把回覆交給模型，示範推論服務怎麼換；第三個換掉規劃機制，示範自訂模組怎麼寫。
...
下面四件事由應用程式負責，這個程式庫不做。四項的實作方式都隨部署環境而不同，因此留在程式庫之外。
