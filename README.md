...
做一個會查資料再回答的 AI agent，難的往往不是自己想解的那一段，而是為了讓那一段跑起來，得先把前後所有環節都接出來——聽懂問題、決定下一步、查資料、產生回覆、檢查結果，缺一個就跑不完一輪。Agentic SDK 把這五個環節都做成現成的 Python 物件，接線的事交給程式庫，開發者只需要把想自己動手的那一個換掉，其餘四個照舊運作。
