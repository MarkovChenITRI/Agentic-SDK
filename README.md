...
用 Python 把一次問答拆成五個步驟，接成一條可以逐步替換的流程。適合做內部文件問答、客服機器人、多輪對話助理這類需要「先查資料再回答」的東西。
...
| 播放聲音、擷取麥克風 | 音訊裝置由應用程式接，SDK 只產生與接收音訊資料 |
